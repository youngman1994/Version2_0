...
# Form implementation generated from reading ui file 'collect_sample2_1.ui'
#
...
"font: 24pt \"黑体\";")
        ...
        self.radioButton_man.setAutoRepeat(False)
        self.radioButton_man.setObjectName("radioButton_man")
        self.horizontalLayout_3.addWidget(self.radioButton_man)
        ...
        self.horizontalLayout_3.addWidget(self.radioButton_woman)
        self.widget1 = QtWidgets.QWidget(Dialog)
        self.widget1.setGeometry(QtCore.QRect(497, 588, 181, 29))
        self.widget1.setObjectName("widget1")
        self.horizontalLayout_4 = QtWidgets.QHBoxLayout(self.widget1)
        self.horizontalLayout_4.setContentsMargins(0, 0, 0, 0)
        self.horizontalLayout_4.setObjectName("horizontalLayout_4")
        self.radioButton_l = QtWidgets.QRadioButton(self.widget1)
        self.radioButton_l.setStyleSheet("font: 12pt \"Arial\";")
        self.radioButton_l.setObjectName("radioButton_l")
        self.horizontalLayout_4.addWidget(self.radioButton_l)
        self.radioButton_r = QtWidgets.QRadioButton(self.widget1)
    ...
    def save_image(self):
        ...
        if flag2 and flag1 and flag3 and flag4 and flag5:#只有当信息都填写完整才会保存
            ...
            if self.radioButton_l.isChecked():#如果是左手
                ...
                Left_text.write(user_data)
                Left_text.write('\n')
                for _num in range(1,len(self.img_list_h)+1):
                    imgname_h = str(self.id) + '_' +'h' + str(_num)+ '.jpg'
                    imgname_k = str(self.id) + '_' + 'k' + str(_num) + '.jpg'
                    tempimg_h = cv.cvtColor(self.img_list_h[(len(self.img_list_h) - _num - 1)], cv.COLOR_RGB2BGR)
                    cv.imencode('.jpg', tempimg_h)[1].tofile('./H/'+imgname_h)
                    temppath_h = targetpath + os.path.sep +'Left_hand'+os.path.sep+'H'+os.path.sep+ imgname_h + '#'
                    Left_text.write(temppath_h)#写左手，红外
                    #写左手，可见
                    tempimg_k = cv.cvtColor(self.img_list_k[(len(self.img_list_k) - _num - 1)], cv.COLOR_RGB2BGR)
                    cv.imencode('.jpg', tempimg_k)[1].tofile('./K/'+imgname_k)
                    temppath_k = targetpath + os.path.sep + 'Left_hand'  +os.path.sep+'K' +os.path.sep+ imgname_k
                ...
                for _num in range(1,len(self.img_list_h)+1):
                    ...
                    tempimg_h = cv.cvtColor(self.img_list_h[(len(self.img_list_h) - _num - 1)], cv.COLOR_RGB2BGR)
                    cv.imencode('.jpg', tempimg_h)[1].tofile('./H/'+imgname_h)
                    temppath_h = targetpath + os.path.sep +'Right_hand'+os.path.sep+'H'+os.path.sep+ imgname_h + '#'
                    Right_text.write(temppath_h)#写右手，红外
                    #写右手，可见
                    tempimg_k = cv.cvtColor(self.img_list_k[(len(self.img_list_k) - _num - 1)], cv.COLOR_RGB2BGR)
                    cv.imencode('.jpg', tempimg_k)[1].tofile('./K/'+imgname_k)
                    temppath_k = targetpath + os.path.sep + 'Right_hand' + os.path.sep+'K' +os.path.sep+ imgname_k
